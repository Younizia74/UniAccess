...
"""
Module d'intégration pour GNOME Terminal
Gère l'interaction avec GNOME Terminal via AT-SPI, incluant :
...
# Rôles spécifiques à GNOME Terminal
GNOME_TERMINAL_ROLES = {
...
_gnome_terminal_instance = None

def initialize() -> bool:
    """Initialise l'intégration avec GNOME Terminal."""
    global _accessibility_manager, _gnome_terminal_instance
    
    try:
        # Initialiser AT-SPI
        _accessibility_manager = Atspi.Accessible()
        
        # Trouver l'instance de GNOME Terminal
        _gnome_terminal_instance = find_gnome_terminal_instance()
        if not _gnome_terminal_instance:
            logger.error("GNOME Terminal n'est pas en cours d'exécution")
            return False
            
        logger.info("Intégration GNOME Terminal initialisée avec succès")
        return True
        
    except Exception as e:
        logger.error(f"Erreur lors de l'initialisation de GNOME Terminal : {str(e)}")
        return False
...
def cleanup() -> None:
    ...
    try:
        _gnome_terminal_instance = None
        _accessibility_manager = None
        logger.info("Intégration GNOME Terminal nettoyée")
    except Exception as e:
        logger.error(f"Erreur lors du nettoyage de GNOME Terminal : {str(e)}")

def find_gnome_terminal_instance() -> Optional[Atspi.Accessible]:
    """Trouve l'instance de GNOME Terminal en cours d'exécution."""
    try:
        desktop = Atspi.get_desktop(0)
        for app in desktop:
            if app.get_name().lower() == 'gnome-terminal':
                return app
        return None
    except Exception as e:
        logger.error(f"Erreur lors de la recherche de GNOME Terminal : {str(e)}")
        return None

def get_instance() -> Optional[Atspi.Accessible]:
    """Récupère l'instance de GNOME Terminal."""
    return _gnome_terminal_instance

def get_terminal_info() -> Dict[str, Any]:
    """Récupère les informations sur l'instance de GNOME Terminal."""
    if not _gnome_terminal_instance:
        return {}
        
    try:
        return {
            'name': _gnome_terminal_instance.get_name(),
            'role': _gnome_terminal_instance.get_role_name(),
            'version': _gnome_terminal_instance.get_attributes().get('version', ''),
            'pid': _gnome_terminal_instance.get_process_id(),
            'children': len(_gnome_terminal_instance.get_children())
        }
    except Exception as e:
        logger.error(f"Erreur lors de la récupération des informations GNOME Terminal : {str(e)}")
        return {}
...
def get_tabs() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        find_tabs(_gnome_terminal_instance)
        return tabs
    except Exception as e:
...
def get_current_tab() -> Dict[str, Any]:
    ...
    try:
        def find_current_tab(element: Atspi.Accessible) -> Optional[Dict[str, Any]]:
            if (element.get_role() == Atspi.Role.PAGE_TAB and 
            ...
            for child in element.get_children():
                result = find_current_tab(child)
                if result:
                    return result
            return None
            
        return find_current_tab(_gnome_terminal_instance) or {}
    except Exception as e:
...
def get_cursor_position() -> Tuple[int, int]:
    ...
    try:
        ...
        return find_cursor_position(_gnome_terminal_instance) or (0, 0)
    except Exception as e:
...
def execute_action(action: str, **kwargs) -> bool:
    ...
    try:
        if action == 'new_tab':
            ...
            for element in _gnome_terminal_instance.get_children():
                if (element.get_role() == Atspi.Role.PUSH_BUTTON and 
                    'new tab' in element.get_name().lower()):
            ...
            for element in _gnome_terminal_instance.get_children():
                if (element.get_role() == Atspi.Role.PUSH_BUTTON and 
                    'close tab' in element.get_name().lower()):
                    return element.do_action(0)  # Action par défaut (clic)
            ...
            for element in _gnome_terminal_instance.get_children():
                if (element.get_role() == Atspi.Role.PUSH_BUTTON and 
                    'next tab' in element.get_name().lower()):
                    return element.do_action(0)  # Action par défaut (clic)
            ...
            for element in _gnome_terminal_instance.get_children():
                if (element.get_role() == Atspi.Role.PUSH_BUTTON and 
                    'previous tab' in element.get_name().lower()):
                    return element.do_action(0)  # Action par défaut (clic)
