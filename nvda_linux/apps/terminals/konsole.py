...
"""
Module d'intégration pour Konsole
Gère l'interaction avec Konsole via AT-SPI, incluant :
...
_konsole_instance = None

def initialize() -> bool:
    """Initialise l'intégration avec Konsole."""
    global _accessibility_manager, _konsole_instance
    
    try:
        # Initialiser AT-SPI
        _accessibility_manager = Atspi.Accessible()
        
        # Trouver l'instance de Konsole
        _konsole_instance = find_konsole_instance()
        if not _konsole_instance:
            logger.error("Konsole n'est pas en cours d'exécution")
            return False
            
        logger.info("Intégration Konsole initialisée avec succès")
        return True
        
    except Exception as e:
        logger.error(f"Erreur lors de l'initialisation de Konsole : {str(e)}")
        return False
...
def cleanup() -> None:
    ...
    try:
        _konsole_instance = None
        _accessibility_manager = None
        logger.info("Intégration Konsole nettoyée")
    except Exception as e:
        logger.error(f"Erreur lors du nettoyage de Konsole : {str(e)}")

def find_konsole_instance() -> Optional[Atspi.Accessible]:
    """Trouve l'instance de Konsole en cours d'exécution."""
    try:
        desktop = Atspi.get_desktop(0)
        for app in desktop:
            if app.get_name().lower() == 'konsole':
                return app
        return None
    except Exception as e:
        logger.error(f"Erreur lors de la recherche de Konsole : {str(e)}")
        return None

def get_instance() -> Optional[Atspi.Accessible]:
    """Récupère l'instance de Konsole."""
    return _konsole_instance

def get_terminal_info() -> Dict[str, Any]:
    """Récupère les informations sur l'instance de Konsole."""
    if not _konsole_instance:
        return {}
        
    try:
        return {
            'name': _konsole_instance.get_name(),
            'role': _konsole_instance.get_role_name(),
            'version': _konsole_instance.get_attributes().get('version', ''),
            'pid': _konsole_instance.get_process_id(),
            'children': len(_konsole_instance.get_children())
        }
    except Exception as e:
        logger.error(f"Erreur lors de la récupération des informations Konsole : {str(e)}")
        return {}
...
def get_current_tab() -> Dict[str, Any]:
    ...
    try:
        ...
        return find_current_tab(_konsole_instance) or {}
    except Exception as e:
...
def get_cursor_position() -> Tuple[int, int]:
    ...
    try:
        ...
        return find_cursor_position(_konsole_instance) or (0, 0)
    except Exception as e:
...
def execute_action(action: str, **kwargs) -> bool:
    ...
    try:
        if action == 'new_tab':
            ...
            for element in _konsole_instance.get_children():
