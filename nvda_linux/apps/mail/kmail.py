...
"""
Module d'intégration pour KMail
Gère l'interaction avec KMail via AT-SPI, incluant :
...
_kmail_instance = None

def initialize() -> bool:
    """Initialise l'intégration avec KMail."""
    global _accessibility_manager, _kmail_instance
    
    try:
        # Initialiser AT-SPI
        _accessibility_manager = Atspi.Accessible()
        
        # Trouver l'instance de KMail
        _kmail_instance = find_kmail_instance()
        if not _kmail_instance:
            logger.error("KMail n'est pas en cours d'exécution")
            return False
            
        logger.info("Intégration KMail initialisée avec succès")
        return True
        
    except Exception as e:
        logger.error(f"Erreur lors de l'initialisation de KMail : {str(e)}")
        return False
...
def cleanup() -> None:
    ...
    try:
        _kmail_instance = None
        _accessibility_manager = None
        logger.info("Intégration KMail nettoyée")
    except Exception as e:
        logger.error(f"Erreur lors du nettoyage de KMail : {str(e)}")

def find_kmail_instance() -> Optional[Atspi.Accessible]:
    """Trouve l'instance de KMail en cours d'exécution."""
    try:
        desktop = Atspi.get_desktop(0)
        for app in desktop:
            if app.get_name().lower() in ['kmail', 'kontact']:
                return app
        return None
    except Exception as e:
        logger.error(f"Erreur lors de la recherche de KMail : {str(e)}")
        return None

def get_mail_info() -> Dict[str, Any]:
    """Récupère les informations sur l'instance de KMail."""
    if not _kmail_instance:
        return {}
        
    try:
        return {
            'name': _kmail_instance.get_name(),
            'role': _kmail_instance.get_role_name(),
            'version': _kmail_instance.get_attributes().get('version', ''),
            'pid': _kmail_instance.get_process_id(),
            'children': len(_kmail_instance.get_children())
        }
    except Exception as e:
        logger.error(f"Erreur lors de la récupération des informations KMail : {str(e)}")
        return {}
...
def get_message_content(message_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return find_message(_kmail_instance) or {}
    except Exception as e:
...
def compose_message(**kwargs) -> bool:
    ...
    try:
        ...
        for element in _kmail_instance.get_children():
...
def reply_to_message(message_id: str, reply_all: bool = False) -> bool:
    ...
    try:
        ...
        message = find_message(_kmail_instance)
        if not message:
            return False
...
def search_messages(query: str, folder: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        search_field = find_search_field(_kmail_instance)
        if not search_field:
            return []
