...
"""
Module d'intégration pour Evolution
Gère l'interaction avec Evolution via AT-SPI, incluant :
...
def initialize() -> bool:
    """Initialise l'intégration avec Evolution."""
    global _accessibility_manager, _evolution_instance
    
    try:
        # Initialiser AT-SPI
        _accessibility_manager = Atspi.Accessible()
        
        # Trouver l'instance d'Evolution
        _evolution_instance = find_evolution_instance()
        if not _evolution_instance:
            logger.error("Evolution n'est pas en cours d'exécution")
            return False
            
        logger.info("Intégration Evolution initialisée avec succès")
        return True
        
    except Exception as e:
        logger.error(f"Erreur lors de l'initialisation d'Evolution : {str(e)}")
        return False
...
def cleanup() -> None:
    ...
    try:
        _evolution_instance = None
        _accessibility_manager = None
        logger.info("Intégration Evolution nettoyée")
    except Exception as e:
        logger.error(f"Erreur lors du nettoyage d'Evolution : {str(e)}")

def find_evolution_instance() -> Optional[Atspi.Accessible]:
    """Trouve l'instance d'Evolution en cours d'exécution."""
    try:
        desktop = Atspi.get_desktop(0)
        for app in desktop:
            if app.get_name().lower() in ['evolution', 'evolution-mail']:
                return app
        return None
    except Exception as e:
        logger.error(f"Erreur lors de la recherche d'Evolution : {str(e)}")
        return None

def get_mail_info() -> Dict[str, Any]:
    """Récupère les informations sur l'instance d'Evolution."""
    if not _evolution_instance:
        return {}
        
    try:
        return {
            'name': _evolution_instance.get_name(),
            'role': _evolution_instance.get_role_name(),
            'version': _evolution_instance.get_attributes().get('version', ''),
            'pid': _evolution_instance.get_process_id(),
            'children': len(_evolution_instance.get_children())
        }
    except Exception as e:
        logger.error(f"Erreur lors de la récupération des informations Evolution : {str(e)}")
        return {}
...
def get_folders() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        find_folders(_evolution_instance)
        return folders
    except Exception as e:
...
def get_messages(folder: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        find_messages(_evolution_instance)
        return messages
    except Exception as e:
...
def get_message_content(message_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return find_message(_evolution_instance) or {}
    except Exception as e:
...
def compose_message(**kwargs) -> bool:
    ...
    try:
        ...
        for element in _evolution_instance.get_children():
...
def reply_to_message(message_id: str, reply_all: bool = False) -> bool:
    ...
    try:
        ...
        message = find_message(_evolution_instance)
        if not message:
            return False
...
def search_messages(query: str, folder: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        search_field = find_search_field(_evolution_instance)
        if not search_field:
            return []
...
def execute_action(action: str, **kwargs) -> bool:
    """Exécute une action dans Evolution."""
    if not _evolution_instance:
        return False
