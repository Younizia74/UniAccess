...
"""
Module d'intégration pour Geary
Gère l'interaction avec Geary via AT-SPI, incluant :
...
_geary_instance = None

def initialize() -> bool:
    """Initialise l'intégration avec Geary."""
    global _accessibility_manager, _geary_instance
    
    try:
        # Initialiser AT-SPI
        _accessibility_manager = Atspi.Accessible()
        
        # Trouver l'instance de Geary
        _geary_instance = find_geary_instance()
        if not _geary_instance:
            logger.error("Geary n'est pas en cours d'exécution")
            return False
            
        logger.info("Intégration Geary initialisée avec succès")
        return True
        
    except Exception as e:
        logger.error(f"Erreur lors de l'initialisation de Geary : {str(e)}")
        return False
...
def cleanup() -> None:
    ...
    try:
        _geary_instance = None
        _accessibility_manager = None
        logger.info("Intégration Geary nettoyée")
    except Exception as e:
        logger.error(f"Erreur lors du nettoyage de Geary : {str(e)}")

def find_geary_instance() -> Optional[Atspi.Accessible]:
    """Trouve l'instance de Geary en cours d'exécution."""
    try:
        desktop = Atspi.get_desktop(0)
        for app in desktop:
            if app.get_name().lower() in ['geary', 'geary-mail']:
                return app
        return None
    except Exception as e:
        logger.error(f"Erreur lors de la recherche de Geary : {str(e)}")
        return None

def get_mail_info() -> Dict[str, Any]:
    """Récupère les informations sur l'instance de Geary."""
    if not _geary_instance:
        return {}
        
    try:
        return {
            'name': _geary_instance.get_name(),
            'role': _geary_instance.get_role_name(),
            'version': _geary_instance.get_attributes().get('version', ''),
            'pid': _geary_instance.get_process_id(),
            'children': len(_geary_instance.get_children())
        }
    except Exception as e:
        logger.error(f"Erreur lors de la récupération des informations Geary : {str(e)}")
        return {}
...
def get_message_content(message_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return find_message(_geary_instance) or {}
    except Exception as e:
...
def compose_message(**kwargs) -> bool:
    ...
    try:
        ...
        for element in _geary_instance.get_children():
...
def reply_to_message(message_id: str, reply_all: bool = False) -> bool:
    ...
    try:
        ...
        message = find_message(_geary_instance)
        if not message:
            return False
...
def search_messages(query: str, folder: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        search_field = find_search_field(_geary_instance)
        if not search_field:
            return []
