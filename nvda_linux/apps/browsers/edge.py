...
"""
Module d'intégration pour Microsoft Edge
Gère l'interaction avec Edge via AT-SPI, incluant :
...
def initialize() -> bool:
    """Initialise l'intégration avec Edge."""
    global _accessibility_manager, _edge_instances
    
    try:
        # Initialiser AT-SPI
        _accessibility_manager = Atspi.Accessible()
        
        # Trouver les instances de Edge
        _edge_instances = find_edge_instances()
        if not _edge_instances:
            logger.warning("Aucune instance de Edge n'est en cours d'exécution")
            return False
            
        logger.info(f"Intégration Edge initialisée avec succès pour {len(_edge_instances)} instances")
        return True
        
    except Exception as e:
        logger.error(f"Erreur lors de l'initialisation de Edge : {str(e)}")
        return False
...
def cleanup() -> None:
    ...
    try:
        _edge_instances.clear()
        _accessibility_manager = None
        logger.info("Intégration Edge nettoyée")
    except Exception as e:
        logger.error(f"Erreur lors du nettoyage de Edge : {str(e)}")

def find_edge_instances() -> Dict[str, Atspi.Accessible]:
    """Trouve les instances de Edge en cours d'exécution."""
    instances = {}
    try:
        desktop = Atspi.get_desktop(0)
        for app in desktop:
            # Vérifier si l'application est Edge
            if is_edge_instance(app):
                instances[app.get_name()] = app
        return instances
    except Exception as e:
        logger.error(f"Erreur lors de la recherche des instances de Edge : {str(e)}")
        return {}

def is_edge_instance(app: Atspi.Accessible) -> bool:
    """Vérifie si une application est une instance de Edge."""
    try:
        # Vérifier le nom et le rôle
        if app.get_name().lower() not in ['microsoft edge', 'edge']:
            return False
        ...
        try:
            with open(f'/proc/{pid}/cmdline', 'r') as f:
                ...
                return 'msedge' in cmdline or 'edge' in cmdline
        except:
            return False
...
def get_edge_info(instance_name: Optional[str] = None) -> Dict[str, Any]:
    """Récupère les informations sur l'instance de Edge spécifiée ou toutes les instances."""
    if instance_name:
        instance = _edge_instances.get(instance_name)
        if not instance:
            return {}
        ...
        return {name: get_edge_info(name) for name in _edge_instances.keys()}

def get_tabs(instance: Atspi.Accessible) -> List[Atspi.Accessible]:
    """Récupère la liste des onglets d'une instance de Edge."""
    tabs = []
    try:
...
def get_accessibility_tree(instance_name: str) -> Dict[str, Any]:
    """Récupère l'arbre d'accessibilité d'une instance de Edge."""
    instance = _edge_instances.get(instance_name)
    if not instance:
        return {}
...
def get_focused_element(instance_name: str) -> Dict[str, Any]:
    """Récupère l'élément actuellement focalisé dans une instance de Edge."""
    instance = _edge_instances.get(instance_name)
    if not instance:
        return {}
...
def get_current_selection(instance_name: str) -> List[Dict[str, Any]]:
    """Récupère la sélection actuelle dans une instance de Edge."""
    instance = _edge_instances.get(instance_name)
    if not instance:
        return []
...
def execute_action(instance_name: str, action: str, **kwargs) -> bool:
    """Exécute une action dans une instance de Edge."""
    instance = _edge_instances.get(instance_name)
    if not instance:
        return False
