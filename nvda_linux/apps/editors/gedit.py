...
"""
Module d'intégration pour Gedit
Gère l'interaction avec Gedit via AT-SPI, incluant :
...
_gedit_instance = None

def initialize() -> bool:
    """Initialise l'intégration avec Gedit."""
    global _accessibility_manager, _gedit_instance
    
    try:
        # Initialiser AT-SPI
        _accessibility_manager = Atspi.Accessible()
        
        # Trouver l'instance de Gedit
        _gedit_instance = find_gedit_instance()
        if not _gedit_instance:
            logger.error("Gedit n'est pas en cours d'exécution")
            return False
            
        logger.info("Intégration Gedit initialisée avec succès")
        return True
        
    except Exception as e:
        logger.error(f"Erreur lors de l'initialisation de Gedit : {str(e)}")
        return False
...
def cleanup() -> None:
    ...
    try:
        _gedit_instance = None
        _accessibility_manager = None
        logger.info("Intégration Gedit nettoyée")
    except Exception as e:
        logger.error(f"Erreur lors du nettoyage de Gedit : {str(e)}")

def find_gedit_instance() -> Optional[Atspi.Accessible]:
    """Trouve l'instance de Gedit en cours d'exécution."""
    try:
        desktop = Atspi.get_desktop(0)
        for app in desktop:
            if app.get_name().lower() == 'gedit':
                return app
        return None
    except Exception as e:
        logger.error(f"Erreur lors de la recherche de Gedit : {str(e)}")
        return None

def get_instance() -> Optional[Atspi.Accessible]:
    """Récupère l'instance de Gedit."""
    return _gedit_instance

def get_editor_info() -> Dict[str, Any]:
    """Récupère les informations sur l'instance de Gedit."""
    if not _gedit_instance:
        return {}
        
    try:
        return {
            'name': _gedit_instance.get_name(),
            'role': _gedit_instance.get_role_name(),
            'version': _gedit_instance.get_attributes().get('version', ''),
            'pid': _gedit_instance.get_process_id(),
            'children': len(_gedit_instance.get_children())
        }
    except Exception as e:
        logger.error(f"Erreur lors de la récupération des informations Gedit : {str(e)}")
        return {}
...
def get_documents() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        find_documents(_gedit_instance)
        return documents
    except Exception as e:
...
def get_current_document() -> Dict[str, Any]:
    ...
    try:
        ...
        return find_current_document(_gedit_instance) or {}
    except Exception as e:
...
def get_cursor_position() -> Tuple[int, int]:
    ...
    try:
        ...
        return find_cursor_position(_gedit_instance) or (0, 0)
    except Exception as e:
...
def execute_action(action: str, **kwargs) -> bool:
    ...
    try:
        if action == 'new':
            ...
            for element in _gedit_instance.get_children():
