...
"""
Module d'intégration pour VSCode
Gère l'interaction avec VSCode via AT-SPI, incluant :
...
VSCODE_ROLES = {
    ...
    'menu_bar': Atspi.Role.MENU_BAR,
    'popup_menu': Atspi.Role.POPUP_MENU,
    'progress_bar': Atspi.Role.PROGRESS_BAR,
    'slider': Atspi.Role.SLIDER,
    'spin_button': Atspi.Role.SPIN_BUTTON,
    'table': Atspi.Role.TABLE,
    'table_cell': Atspi.Role.TABLE_CELL,
    'table_row': Atspi.Role.TABLE_ROW,
    'toggle_button': Atspi.Role.TOGGLE_BUTTON,
    'tool_bar': Atspi.Role.TOOL_BAR,
    'tool_bar_item': Atspi.Role.TOOL_BAR_ITEM,
    'tree_table': Atspi.Role.TREE_TABLE,
    'tree_table_cell': Atspi.Role.TREE_TABLE_CELL,
    'tree_table_row': Atspi.Role.TREE_TABLE_ROW
}

# Variables globales
_accessibility_manager = None
_vscode_instance = None

def initialize() -> bool:
    """Initialise l'intégration avec VSCode."""
    global _accessibility_manager, _vscode_instance
    
    try:
        # Initialiser AT-SPI
        _accessibility_manager = Atspi.Accessible()
        
        # Trouver l'instance de VSCode
        _vscode_instance = find_vscode_instance()
        if not _vscode_instance:
            logger.error("VSCode n'est pas en cours d'exécution")
            return False
            
        logger.info("Intégration VSCode initialisée avec succès")
        return True
        
    except Exception as e:
        logger.error(f"Erreur lors de l'initialisation de VSCode : {str(e)}")
        return False
...
def cleanup() -> None:
    ...
    try:
        _vscode_instance = None
        _accessibility_manager = None
        logger.info("Intégration VSCode nettoyée")
    except Exception as e:
        logger.error(f"Erreur lors du nettoyage de VSCode : {str(e)}")

def find_vscode_instance() -> Optional[Atspi.Accessible]:
    """Trouve l'instance de VSCode en cours d'exécution."""
    try:
        desktop = Atspi.get_desktop(0)
        for app in desktop:
            if app.get_name().lower() == 'code':
                return app
        return None
    except Exception as e:
        logger.error(f"Erreur lors de la recherche de VSCode : {str(e)}")
        return None

def get_instance() -> Optional[Atspi.Accessible]:
    """Récupère l'instance de VSCode."""
    return _vscode_instance

def get_editor_info() -> Dict[str, Any]:
    """Récupère les informations sur l'instance de VSCode."""
    if not _vscode_instance:
        return {}
        
    try:
        return {
            'name': _vscode_instance.get_name(),
            'role': _vscode_instance.get_role_name(),
            'version': _vscode_instance.get_attributes().get('version', ''),
            'pid': _vscode_instance.get_process_id(),
            'children': len(_vscode_instance.get_children())
        }
    except Exception as e:
        logger.error(f"Erreur lors de la récupération des informations VSCode : {str(e)}")
        return {}
        ...
        def find_documents(element: Atspi.Accessible) -> None:
            if element.get_role() == Atspi.Role.DOCUMENT_FRAME:
                documents.append({
                    ...
                    'folded': element.get_attributes().get('folded', 'false') == 'true',
                    'git_status': element.get_attributes().get('git_status', ''),
                    'git_branch': element.get_attributes().get('git_branch', '')
                })
            for child in element.get_children():
                find_documents(child)
                
        find_documents(_vscode_instance)
        return documents
    except Exception as e:
...
def get_current_document() -> Dict[str, Any]:
    ...
    try:
        ...
        return find_current_document(_vscode_instance) or {}
    except Exception as e:
...
def get_cursor_position() -> Tuple[int, int]:
    ...
    try:
        ...
        return find_cursor_position(_vscode_instance) or (0, 0)
    except Exception as e:
...
def execute_action(action: str, **kwargs) -> bool:
    ...
    try:
        if action == 'new':
            ...
            for element in _vscode_instance.get_children():
            ...
            for element in _vscode_instance.get_children():
                if (element.get_role() == Atspi.Role.PUSH_BUTTON and 
                    'minimap' in element.get_name().lower()):
                    return element.do_action(0)  # Action par défaut (clic)
                    
        elif action == 'toggle_word_wrap':
            # Activer/désactiver le retour à la ligne automatique
            for element in _vscode_instance.get_children():
                if (element.get_role() == Atspi.Role.PUSH_BUTTON and 
                    'word wrap' in element.get_name().lower()):
                    return element.do_action(0)  # Action par défaut (clic)
            ...
            for element in _vscode_instance.get_children():
                if (element.get_role() == Atspi.Role.PUSH_BUTTON and 
                    'line numbers' in element.get_name().lower()):
                    return element.do_action(0)  # Action par défaut (clic)
                    
        elif action == 'toggle_folding':
            # Activer/désactiver le pliage du code
            for element in _vscode_instance.get_children():
                if (element.get_role() == Atspi.Role.PUSH_BUTTON and 
                    'folding' in element.get_name().lower()):
                    return element.do_action(0)  # Action par défaut (clic)
                    
        elif action == 'toggle_git':
            # Afficher/masquer le panneau Git
            for element in _vscode_instance.get_children():
                if (element.get_role() == Atspi.Role.PUSH_BUTTON and 
                    'git' in element.get_name().lower()):
                    return element.do_action(0)  # Action par défaut (clic)
                    
        elif action == 'toggle_extensions':
            # Afficher/masquer le panneau des extensions
            for element in _vscode_instance.get_children():
                if (element.get_role() == Atspi.Role.PUSH_BUTTON and 
                    'extensions' in element.get_name().lower()):
                    return element.do_action(0)  # Action par défaut (clic)
            ...
            for element in _vscode_instance.get_children():
                if (element.get_role() == Atspi.Role.PUSH_BUTTON and 
                    'settings' in element.get_name().lower()):
                    return element.do_action(0)  # Action par défaut (clic)
