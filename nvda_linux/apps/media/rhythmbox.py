...
"""
Module d'intégration pour Rhythmbox
Gère l'interaction avec Rhythmbox via AT-SPI, incluant :
...
_rhythmbox_instance = None

def initialize() -> bool:
    """Initialise l'intégration avec Rhythmbox."""
    global _accessibility_manager, _rhythmbox_instance
    
    try:
        # Initialiser AT-SPI
        _accessibility_manager = Atspi.Accessible()
        
        # Trouver l'instance de Rhythmbox
        _rhythmbox_instance = find_rhythmbox_instance()
        if not _rhythmbox_instance:
            logger.error("Rhythmbox n'est pas en cours d'exécution")
            return False
            
        logger.info("Intégration Rhythmbox initialisée avec succès")
        return True
        
    except Exception as e:
        logger.error(f"Erreur lors de l'initialisation de Rhythmbox : {str(e)}")
        return False
...
def cleanup() -> None:
    ...
    try:
        _rhythmbox_instance = None
        _accessibility_manager = None
        logger.info("Intégration Rhythmbox nettoyée")
    except Exception as e:
        logger.error(f"Erreur lors du nettoyage de Rhythmbox : {str(e)}")

def find_rhythmbox_instance() -> Optional[Atspi.Accessible]:
    """Trouve l'instance de Rhythmbox en cours d'exécution."""
    try:
        desktop = Atspi.get_desktop(0)
        for app in desktop:
            if app.get_name().lower() == 'rhythmbox':
                return app
        return None
    except Exception as e:
        logger.error(f"Erreur lors de la recherche de Rhythmbox : {str(e)}")
        return None

def get_instance() -> Optional[Atspi.Accessible]:
    """Récupère l'instance de Rhythmbox."""
    return _rhythmbox_instance

def get_player_info() -> Dict[str, Any]:
    """Récupère les informations sur l'instance de Rhythmbox."""
    if not _rhythmbox_instance:
        return {}
        
    try:
        return {
            'name': _rhythmbox_instance.get_name(),
            'role': _rhythmbox_instance.get_role_name(),
            'version': _rhythmbox_instance.get_attributes().get('version', ''),
            'pid': _rhythmbox_instance.get_process_id(),
            'children': len(_rhythmbox_instance.get_children())
        }
    except Exception as e:
        logger.error(f"Erreur lors de la récupération des informations Rhythmbox : {str(e)}")
        return {}
...
def get_library() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        find_library(_rhythmbox_instance)
        return library
    except Exception as e:
...
def get_playlists() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        find_playlists(_rhythmbox_instance)
        return playlists
    except Exception as e:
...
def get_current_track() -> Dict[str, Any]:
    ...
    try:
        ...
        return find_current_track(_rhythmbox_instance) or {}
    except Exception as e:
...
def get_playback_state() -> Dict[str, Any]:
    ...
    try:
        ...
        return find_playback_state(_rhythmbox_instance)
    except Exception as e:
...
def play() -> bool:
    ...
    try:
        ...
        for element in _rhythmbox_instance.get_children():
...
def add_to_playlist(playlist_id: str, track_id: str) -> bool:
    ...
    try:
        ...
        playlist = find_playlist(_rhythmbox_instance)
        if not playlist:
            return False
        ...
        track = find_track(_rhythmbox_instance)
        if not track:
            return False
...
def execute_action(action: str, **kwargs) -> bool:
    """Exécute une action dans Rhythmbox."""
    if not _rhythmbox_instance:
        return False
